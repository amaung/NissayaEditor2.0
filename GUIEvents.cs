        ...
        */
        private void tableToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (newDesign)
            {
                ...
                button_Find.Enabled = true;
                curViewCode = 4; currentViewMenu = ViewMenu.Tabular;
        ...
        private void aboutToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //var e = rtb.KeyDown;
            string server = clientTipitakaDB.GetCloudStorageName();
            if (formLogin != null && formLogin.clientTipitakaDBLogin != null &&
                formLogin.clientTipitakaDBLogin.loggedinUser != null)
            {
                about.SetServerName(String.Format("\"{0}\" --- connected.", server));
            }
            else about.SetServerName(String.Format("\"{0}\" --- not connected.", server));

            //else
            //{
            //    if (LocalLoginUserID.Length > 0 && LocalLoginPassword.Length > 0) about.SetServerName("Local server");
            //    else about.SetServerName("Not connected.");
            //}
            if (about != null)
                about.ShowDialog();
        }
...
#pragma warning restore CA1416
        ...
        {
            if (tabControl1.SelectedIndex < 0 || tabControl1.SelectedIndex >= nissayaTabPageList.Count)
            {
                MessageBox.Show("Save encountered NissayaTabControl selected index out of range. " + tabControl1.SelectedIndex.ToString());
                return;
            }
            NissayaTabPage ntp = nissayaTabPageList[tabControl1.SelectedIndex];
            RichTextBox rtb = ntp.GetRichTextBox();
            if (rtb.Text.Length == 0 && ntp.GetDataGridView().Rows.Count == 1)
            {
                MessageBox.Show("Nothing to save. No text data or no row.");
                return;
            }
            ...
            //quitAppendMode();
            ntp.SavePage(); // save current page
            ntp.SaveFile();
            pageMenuBar.UpdateTimeSpent(ntp.curPageNo);
        ...
        private void Form1_Closing(object sender, FormClosingEventArgs e)
        {
            ...
            if (form_PageReadingView != null) form_PageReadingView.SaveData();
        }
