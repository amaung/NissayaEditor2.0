...
namespace NissayaEditor
{
    public partial class Form1 : Form
    {
        ...
        public partial class NissayaTabPage //: TabPage
        {
            ...
            public NissayaDataGridView GetDataGridView(bool main = true) { return dataGridView1; }
            ...
            public bool UpdateNotSaved()
                { return dataGridView1_Updated || richTextBox1_Updated || fileUpdated || 
                    (dataInfo != null && dataInfo.UpdateFileContent); }
            public string GetCurrentPage() { return curPageNo; }
            public void SetPageToLoad(string pgno) {  dataInfo.PageToLoad = pgno; }
            public string GetPageToLoad() {  return dataInfo.PageToLoad; }
            public int GetPageNISCount(string pgNo) { return dataInfo.GetNISCount(pgNo); }
...
#pragma warning disable CA1416
                button1.Font = new Font("Microsoft Sans Serif", 8F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
#pragma warning restore CA1416
                ...
                button1.Click += clearDataBox_Click; // parentForm.dataBoxCtrlDToolStripMenuItem_Click;
                ...
                ttp.SetToolTip(button1, "Clear databox.");
...
#pragma warning restore CA1416
                ...
                e.DrawBorder();
#pragma warning disable CA1416
                e.Graphics.DrawString(e.ToolTipText, f, Brushes.Gray, new PointF(8, 4));
#pragma warning restore CA1416
            }
            private void clearDataBox_Click(object sender, EventArgs e)
            {
                dataBox.Text = string.Empty;
                dataBox.Focus();
                dataBox.SelectionStart = dataBox.SelectionLength = 0;
            }
            ...
            public void SavePage()
            {
                ...
                {
                    MessageBox.Show("Error in NissayaTabPage::SavePage() : " + ex.Message);
                }
            }
            ...
            public void SpellCheckGridPageMatches()
            {
                if (tabPage.Text.IndexOf("<New") != -1) return;

                CurActiveGridPageFindInfo = null;       // this is not a Find
                string pgno = string.Empty;
                List<string> curPageList = GetPageList();
                int nn = 0;
                Boolean found = false;
                List<_GridViewSelection> listGridViewSelection = new List<_GridViewSelection>();
                foreach (string pno in curPageList)
                {
                    if (!found && pno == curFindPgNo) found = true;
                    if (found)
                    {
                        listGridViewSelection = dataInfo.SpellCheckGridPageMatches(pno);
                        if (listGridViewSelection.Count > 0)
                        {
                            dataGridView1.FillFindGridMatches(pno, listGridViewSelection, false);
                            parentForm.errorReporter.UpdateTotalCount(listGridViewSelection.Count);
                        }
                    }
                    else ++nn;
                }
                ...
                if (DataGridViewFindPageList != null && DataGridViewFindPageList.Count > 0)
                //if (listGridViewSelection != null && listGridViewSelection.Count > 0)
                {
                    curFindPgNo = curPageNo;
                    string TabPageKey = GetThisTabPageKey(curPageNo);
                    //DataGridViewFindPage dataGridViewFindResults = null;
                    if (!DataGridViewFindPageList.ContainsKey(TabPageKey))
                    //if (curPageNo != curFindPgNo)
                    {
                        TabPageKey = dataGridView1.GetNextTabPageKeyFindPageList(DataGridViewFindPageList, TabPageKey, true);
                        LoadPage(curFindPgNo);
                        parentForm.pageMenuBar.NewPageSelected(curFindPgNo);

                    }
                    parentForm.CurPageHighlightType = HightlightType.HighlightSelectedText |
                                                        HightlightType.HighlightText;
                    //dataGridView1.dataGridSpellCheckerNext();
                    dataGridView1.dataGridSpellCheckerNavigate();
                }
                else
                {
                    parentForm.errorReporter.Hide();
